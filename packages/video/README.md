...
仅支持 **B站、抖音、小红书**，可单独安装。需要服务器安装 `ffmpeg` 和 `ffprobe`；配置项同名，可填写可执行文件路径。
...
- `autoParse`：默认关闭。开启后仅自动处理 `groups` 中配置的群；没有匹配链接的消息交还其他插件。
...
每个任务使用独立目录，ffmpeg 不通过 shell 启动且输入只允许 file/pipe；取消/超时会终止子进程并清理。ffprobe 检查视频/音频编码、短边、时长和容器；不发送检测为截断的输出。缓存路径是 Koishi 数据目录下的 `data/ember-video-parser/`，按视频、分 P、清晰度和账号配置隔离。程序被操作系统强制杀死时的旧 `job-*` 目录需要停机后清理。
...
官方 OneBot 适配器 6.9.4 模拟协议确认能够输出 `video` 消息段。用户的 SnowLuma 版本、真实 QQ 视频播放及三站账号条件尚未验收；B站通过不能代表其他两站通过。
